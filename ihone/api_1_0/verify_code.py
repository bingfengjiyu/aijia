...
from ihone import redis_store, constaints, db
from flask import current_app, jsonify, make_response, request
from ihone.utils.response_code import RET
from ihone.libs.yuntongxun.sms import CCP
import random
...
def send_sms_code(mobile):
    ...
    # 保存短信验证码
    try:
        redis_store.setex("sms_code_%s" % mobile, constaints.SMS_CODE_REDIS_EXPIRES, sms_code)
    except Exception as e:
        current_app.logger.error(e)
        resp = {
            "errno": RET.DBERR,
    ...
    try:
        ccp = CCP()
        result = ccp.send_template_sms(mobile, [sms_code,str(constaints.SMS_CODE_REDIS_EXPIRES/60)], 1)
    except Exception as e:
        current_app.logger.error(e)
        resp = {
            "errno": RET.THIRDERR,
